...
def test_json_safe_response():
    ...
    # Fixed 2026-07-01: file was truncated here (no except/finally block).
    # Completion is the obvious one given the function name and docstring
    # ("JSON-safe response") -- assert the response actually serializes.
    try:
        json.dumps(out)
    except (TypeError, ValueError) as e:
        pytest.fail(f"API response is not JSON-safe: {e}")