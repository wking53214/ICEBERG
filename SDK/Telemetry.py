...
class TelemetryKernel:
    ...
    def record(self, event_type: str, payload: Dict[str, Any]):
        ...
        Fixed 2026-07-01: previously used time.time() as the timestamp,
        directly contradicting this class's own "deterministic, zero
        stochasticity" claim -- two TelemetryKernels stepping through an
        identical sequence would never produce equal ledgers, which is
        exactly what test_telemetry_deterministic checks. Now uses a
        monotonic event count, which is reproducible by construction.
        Also renamed the event_type key to `type` in the stored dict --
        every consumer (test_cluster_runner.py, ReplayEngine) reads `type`,
        not `event_type`; only the dict output changed, not this method's
        parameter name, so no caller needs to change.
        """
        evt = TelemetryEvent(
            timestamp=len(self.ledger),
            type=event_type,
            payload=payload,
        )
